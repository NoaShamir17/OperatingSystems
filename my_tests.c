...
//checks free error
void testFreeErr();
//checks malloc and free basic functionality, one case of fragmentation
void testBasic1();
//checks malloc and free basic functionality, another case of fragmentation
void testBasic2();
//checks malloc with gap at start of heap, and best fit allocation
void testGapAtStart();
//checks best fit allocation in fragmented heap
void testBestFit();
//checks calloc functionality
void testCalloc();
//checks realloc functionality
void testRealloc();
//checks multi-threaded malloc and free,
// we manualy check if new regions open only when needed by going over the printed output
...
//tests multi-threaded calloc by having each thread allocate an array using calloc,
//then checking that all elements are initialized to zero, then freeing all arrays.
void testMTCalloc();
void* threadCallocRoutine(void* arg);
...
void test_handler(){
	...
	//Transition to Part B - multi-threaded allocator
	heapKill();
	heapCreate();
	testMTMallocFree();
	testMTRealloc();
...
void testMTRealloc(){
    printf("-------------Test MT Realloc----------:\n");
    
    pthread_t threads[THREADS_NUM];
    for(int i = 0; i < THREADS_NUM; i++){
        pthread_create(&threads[i], NULL, threadReallocRoutine, NULL);
    }
    for(int i = 0; i < THREADS_NUM; i++){
        pthread_join(threads[i], NULL);
    }
    printf("End of Test MT Realloc\n");
}
...
void* threadReallocRoutine(void* arg){
    ...
    for(int i = 0; i < ALLOCS_PER_THREAD; i++){
        ...
        ptrs[i] = customMTMalloc(sizes[i]);
        printf("Thread %lu: Initially allocated ptrs[%d] of size %zu at %p\n in region %d\n", pthread_self(), i, sizes[i], ptrs[i], getRegionIndexByAdress((Header*)((size_t)ptrs[i] - sizeof(Header))));
    }
    //reallocations
    for(int i = 0; i < ALLOCS_PER_THREAD; i++){
        size_t newSize = (rand() % MAX_ALLOC_SIZE) + 1;
        void* newPtr = customMTRealloc(ptrs[i], newSize);
        printf("Thread %lu: Reallocated ptrs[%d] from size %zu to size %zu at %p\n in region %d\n", pthread_self(), i, sizes[i], newSize, newPtr, getRegionIndexByAdress((Header*)((size_t)newPtr - sizeof(Header))));
        ptrs[i] = newPtr;
        sizes[i] = newSize;
    }
    //free
...
void testMTCalloc(){
    printf("-------------Test MT Calloc----------:\n");
    
    pthread_t threads[THREADS_NUM];
    for(int i = 0; i < THREADS_NUM; i++){
        pthread_create(&threads[i], NULL, threadCallocRoutine, NULL);
    }
    for(int i = 0; i < THREADS_NUM; i++){
        pthread_join(threads[i], NULL);
    }
    printf("End of Test MT Calloc\n");
}
