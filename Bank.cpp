...
void* investmentThreadRoutine(void* arg) {
    ...
    const double factor = std::pow(1.03, static_cast<double>(timeMillis));
...
void* Bank::vipRoutine(void* arg) {
    ...
    while (true) {
        ...
        while (bank->vipQueue.empty() && bank->isWorking) {
            pthread_cond_wait(&bank->vipCond, &bank->vipMutex);
        }

        if (!bank->isWorking && bank->vipQueue.empty()) {
...
void* Bank::commissionRoutine(void* arg) {
    ...
    while (bank->isWorking) {
        usleep(3030u * 1000u); // 3 seconds + 30 ms
        if (!bank->isWorking) {
            break;
        }
...
void* Bank::statusRoutine(void* arg) {
    ...
    while (bank->isWorking) {
        usleep(510u * 1000u); // 0.5 seconds + 10 ms
        if (!bank->isWorking) {
            break;
        }
